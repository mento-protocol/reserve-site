...
async function fetchBTCBalance() {
  ...
  return getSumBalance(btc.addresses, (address) => {
    return duel(
      getBlockChainBTCBalance(address),
      getBlockStreamBTCBalance(address),
    );
  });
}
...
const getTokenConfig = (token: Tokens, network: Network) => {
  return addressesConfig.find(
    (tc) => tc.token == token && tc.network == network,
  );
};
...
export async function celoCustodiedBalance() {
  return getOrSave<ProviderResult>(
    "celo-custody-balance",
    getInCustodyBalance,
    5 * MINUTE,
  );
}
...
export async function getHoldingsCelo() {
  ...
  celoCustodied.value += await uniV3HoldingsForToken(
    RESERVE_MULTISIG_CELO,
    CELO_ADDRESS,
  );

  const response = {
    celo: toCeloShape(frozen, unfrozen, celoCustodied, celoRate),
  };
  return response;
}

async function uniV3StakedCeloInCelo(address: string): Promise<number> {
  const stCeloBalance = await uniV3HoldingsForToken(
    address,
    STAKED_CELO_ERC20_ADDRESS,
  );
...
export async function getHoldingsOther() {
  ...
  btcHeld.value += await uniV3HoldingsForToken(
    RESERVE_MULTISIG_CELO,
    BTC_AXELAR_ADDRESS,
  );
  btcHeld.value += await uniV3HoldingsForToken(
...
function toToken(
  token: Tokens,
  units: ResultOk<number>,
  rate?: ResultOk<number>,
): TokenModel {
  let rateValue = 1;
  if (rate) {
