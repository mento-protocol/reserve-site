...
export function calculateTargetAllocation(
  ...
  const usdBackedStablesUSDValue =
    totalStablesUSDValue - eurBackedStablesUSDValue;
  ...
  if (ratio >= 1.9) {
    ...
    celoTarget = 0.45;
    otherCollateralTarget =
      (1 - usdCollateralTarget - eurCollateralTarget - celoTarget) * 0.98;
    natCollateralTarget =
      (1 - usdCollateralTarget - eurCollateralTarget - celoTarget) * 0.02;
  ...
  const targetAllocation: Allocation[] = JSON.parse(
    JSON.stringify(EMPTY_TARGETS),
  );
